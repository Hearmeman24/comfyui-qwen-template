...
The flag also pulls `krea2_raw_bf16.safetensors` (the undistilled Krea-2 base, 26 GB) into `models/diffusion_models/`. No workflow ships for it — it's there so you can wire your own graph with normal step counts and CFG, rather than the Turbo settings the two workflows above use.
