...
#
#   1. sync the template repo   /comfyui-qwen-template   -> origin/master
#   2. read runtime_ref from    /comfyui-qwen-template/pins.json
#   3. sync the shared runtime  /comfyui-runtime          -> that pinned ref
#   4. exec bash /comfyui-runtime/src/start.sh /comfyui-qwen-template
#
...
TEMPLATE_DIR=/comfyui-qwen-template
TEMPLATE_URL=https://github.com/Hearmeman24/comfyui-qwen-template.git
TEMPLATE_BRANCH=master    # qwen's default branch is master, not main
...
exec bash /comfyui-runtime/src/start.sh /comfyui-qwen-template
