#!/usr/bin/env bash
# Version guard for pre-migration images (v13 and earlier). NOT the boot script.
#
...
# src/provision_models.py and src/download_models.py (both deleted by the
# migration) emits two "cp: cannot stat" lines to stderr. The old entrypoint
# has no `set -e`, so they are non-fatal, and the banner below dominates the
# log. Do not add stub .py files to suppress them.
...
============================================================
  ...
  To get running again:
    ...
    1. Edit your RunPod template and set the image to:
         hearmeman/comfyui-qwen-template:v14
