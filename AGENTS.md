...
Local validation: `node tools/catalog-contract.mjs --root . --base-ref "$(git rev-parse origin/master)"`. Validator tests: `npm test`.

This repository also owns the shared catalogue-validator package. Changes to its contract require updating the pinned commit in the other template checks and in the platform dependency together.
